...
interface Props {
 ...
 export const ReservationPage16 = () => {
  const navigate = useNavigate();
  const [params, setParams] = useSearchParams();
  const text31 = params.get('q') || '';
    const text32 = params.get('r') || '';
    const SelectColor16 = params.get('color') || '';
   ...
   const okButton = () => {
    if(text31 === "" && text32 === "" && SelectColor16 === "") {
      ...
      localStorage.setItem("text31",JSON.stringify(text31));
     localStorage.setItem("text32",JSON.stringify(text32));
     localStorage.setItem("SelectColor16",JSON.stringify(SelectColor16));
     }
    navigate("/ShowText",{
      state:{
      text31:text31 || '',
      ...
      <div>
        <div>
          ...
          <table border={1}   color="bule">
            ...
            <STr>
               <STh>予約対象</STh>  <td>{Ids.hotStirrer}</td>
            </STr>
            <STr>
               <STh>テキスト</STh>  <td><SInput type="text" value={text31} 
            onChange={textInput} /></td>
            </STr>
            <STr>
               <STh>予約者（登録者）</STh>  <td><SInput  type="text" value={text32} 
               onChange={textInput2} /></td>
            </STr>
            <STr>
               <STh>色</STh> <td><input type="color" value={SelectColor16}  
