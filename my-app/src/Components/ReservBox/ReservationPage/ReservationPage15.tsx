...
interface Props {
 ...
 export const ReservationPage15 = () => {
  ...
  const text29 = params.get('q') || '';
   ...
   const okButton = () => {
    if(text29 === "" && text30 === "" && SelectColor15 === "") {
      ...
      localStorage.setItem("text29",JSON.stringify(text29));
     localStorage.setItem("text30",JSON.stringify(text30));
     localStorage.setItem("SelectColor15",JSON.stringify(SelectColor15));
     }
    navigate("/ShowText",{
      state:{
        text29:text29 || '',
      ...
      <div>
        <div>
          ...
          <table border={1}   color="bule">
            ...
            <STr>
               <STh>予約対象</STh>  <td>{Ids.DMA}</td>
            </STr>
            <STr>
               <STh>テキスト</STh>  <td><SInput type="text" value={text29} 
            onChange={textInput} /></td>
            </STr>
            <STr>
               <STh>予約者（登録者）</STh>  <td><SInput id="text2" type="text" value={text30} 
               onChange={textInput2} /></td>
            </STr>
            <STr>
               <STh>色</STh> <td><input type="color" value={SelectColor15}  
