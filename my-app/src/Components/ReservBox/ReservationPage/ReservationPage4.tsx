...
interface Props {
 ...
 export const ReservationPage4 = () => {
    const navigate = useNavigate();
    const [params, setParams] = useSearchParams();
    const text7 = params.get('q') || '' 
    const text8 = params.get('r') || ''
    const SelectColor4 = params.get('color') || '';
   ...
   const okButton = () => {
    navigate("/ShowText",{
      state:{
        text7:text7 || '',
        text8:text8 || '',
        SelectColor4:SelectColor4 || '',

      }
    })
    if(text7 === "" && text8 === "" && SelectColor4 === "") {
      localStorage.removeItem("text7")
      localStorage.removeItem("text8")
      localStorage.removeItem("SelectColor4")
     }else{
      ...
      <div>
        <div>
          ...
          <table border={1}   color="bule">
            ...
            <STr>
               <STh>テキスト</STh>  <td><SInput type="text" value={text7}
            onChange={textInput} /></td>
            </STr>
            <STr>
               <STh>予約者（登録者）</STh>  <td><SInput  type="text" value={text8}
               onChange={textInput2} /></td>
            </STr>
            <STr>
               <STh>色</STh> <td><input type="color" value={SelectColor4}  
