...
interface Props {
 ...
 export const ReservationPage14 = () => {
  const navigate = useNavigate();
  const [params, setParams] = useSearchParams();
  const text27 = params.get('q') || '';
    const text28 = params.get('r') || '';
    const SelectColor14 = params.get('color') || '';
   ...
   const okButton = () => {
    if(text27 === "" && text28 === "" && SelectColor14 === "") {
      localStorage.removeItem("text27")
      localStorage.removeItem("text28")
      localStorage.removeItem("SelectColor14")
      console.log("localStorageから削除されました")
     }else{
      localStorage.setItem("text27",JSON.stringify(text27));
     localStorage.setItem("text28",JSON.stringify(text28));
     localStorage.setItem("SelectColor14",JSON.stringify(SelectColor14));
     }
    navigate("/ShowText",{
      state:{
        text27:text27 || '',
      ...
      <div>
        <div>
          ...
          <table border={1}   color="bule">
            ...
            <STr>
               <STh>予約対象</STh>  <td>{Ids.DSC60}</td>
            </STr>
            <STr>
               <STh>テキスト</STh>  <td><SInput type="text" value={text27} 
            onChange={textInput} /></td>
            </STr>
            <STr>
               <STh>予約者（登録者）</STh>  <td><SInput id="text2" type="text" value={text28} 
               onChange={textInput2} /></td>
            </STr>
            <STr>
               <STh>色</STh> <td><input type="color" value={SelectColor14}  
