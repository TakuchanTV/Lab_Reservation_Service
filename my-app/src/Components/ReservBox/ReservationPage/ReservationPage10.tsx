...
import { SelectDays } from "../ReservCustomhooks/SelectDays";
 ...
 export const ReservationPage10 = () => {
  const navigate = useNavigate();
  const [params, setParams] = useSearchParams();
  const text19 = params.get('q') || '';
    const text20 = params.get('r') || '';
    const SelectColor10 = params.get('color') || '';
   ...
   const okButton = () => {
    if(text19 === "" && text20 === "" && SelectColor10 === "") {
      localStorage.removeItem("text19")
      localStorage.removeItem("text20")
      localStorage.removeItem("SelectColor10")
      console.log("localStorageから削除されました")
     }else{
      localStorage.setItem("text19",JSON.stringify(text19));
     localStorage.setItem("text20",JSON.stringify(text20));
     localStorage.setItem("SelectColor10",JSON.stringify(SelectColor10));
     }
    navigate("/ShowText",{
      state:{
        text19:text19 || '',
      ...
      <div>
        <div>
          ...
          <table border={1}   color="bule">
            ...
            <STr>
               <STh>予約対象</STh>  <td>{Ids.hotPress}</td>
            </STr>
            <STr>
               <STh>テキスト</STh>  <td><SInput type="text" value={text19} 
            onChange={textInput} /></td>
            </STr>
            <STr>
               <STh>予約者（登録者）</STh>  <td><SInput id="text2" type="text" value={text20} 
               onChange={textInput2} /></td>
            </STr>
            <STr>
               <STh>色</STh> <td><input type="color" value={SelectColor10}  
