...
import { SelectDays } from "../ReservCustomhooks/SelectDays";
 ...
 export const ReservationPage6 = () => {
    const navigate = useNavigate();
    const [params, setParams] = useSearchParams();
    const text11 = params.get('q') || '';
    const text12 = params.get('r') || '';
    const SelectColor6 = params.get('color') || '';
   ...
   const okButton = () => {
    if(text11 === "" && text12 === "" && SelectColor6 === "") {
      localStorage.removeItem("text11")
      localStorage.removeItem("text12")
      localStorage.removeItem("SelectColor6")
     ...
     localStorage.setItem("SelectColor6",JSON.stringify(SelectColor6));
     }
    navigate("/ShowText",{
      state:{
        text11:text11 || '',
      ...
      <div>
        <div>
          ...
          <table border={1}   color="bule">
            ...
            <STr>
               <STh>予約対象</STh>  <td>{Ids.outsideOven}</td>
            </STr>
            <STr>
               <STh>テキスト</STh>  <td><SInput type="text" value={text11} 
            onChange={textInput} /></td>
            </STr>
            <STr>
               <STh>予約者（登録者）</STh>  <td><SInput id="text2" type="text" value={text12} 
               onChange={textInput2} /></td>
            </STr>
            <STr>
               <STh>色</STh> <td><input type="color" value={SelectColor6}  
